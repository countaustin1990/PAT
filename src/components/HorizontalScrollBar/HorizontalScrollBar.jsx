...
import './HorizontalScrollBar.test.css';
...
const HorizontalScrollBar = () => {
  ...
  useEffect(() => {
    ...
    const resumeScroll = () => animationFrameId = requestAnimationFrame(() => scroll(performance.now()));
  ...
  return (
    <div className="container mx-auto my-10">
      ...
      <div ref={scrollRef} className="scrollbar flex space-x-4 overflow-x-hidden py-4 px-4">
        <div className="flex space-x-4">
          {items.concat(items).map((item, index) => (
            <div key={item.id + (index >= items.length ? '-dup' : '')} className="scroll-item bg-red-500 rounded-lg min-w-[300px] min-h-[300px]">
              <img src={item.image} alt={item.title} className="h-56 w-50 rounded-lg object-cover" />
